...
public final class GlassRenderer {
    ...
    public static void recordPanel(GuiGraphicsExtractor context, float x, float y, float width,
        ...
        panel = new Panel(x, y, width, height, radius, light);
    }
    ...
    public static boolean render(Minecraft minecraft) {
        ...
        uploadUniforms(minecraft, mainTarget.width, mainTarget.height);
        runBlur(mainTarget);
    ...
    private static void uploadUniforms(Minecraft minecraft, int width, int height) {
        ...
        try (GpuBufferSlice.MappedView mapped = samplerInfo.map(false, true)) {
            Std140Builder.intoBuffer(mapped.data()).putVec2(width, height).putVec2(width, height);
        }
        try (GpuBufferSlice.MappedView mapped = panelInfo.map(false, true)) {
            ...
            builder.putVec4((float) (System.nanoTime() / 1_000_000_000.0), 13.0f, 0.26f, 5.0f);
        }
    ...
    private static void composite(RenderTarget mainTarget) {
        try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                () -> "frosty liquid glass", mainTarget.getColorTextureView(), Optional.empty(),
    ...
    private static void draw(CommandEncoder encoder, GpuTextureView output,
                             RenderPipeline pipeline, GpuBuffer config, String samplerName, GpuTextureView input) {
        try (RenderPass pass = encoder.createRenderPass(() -> "frosty liquid glass blur", output, Optional.empty())) {
    ...
    private static void drawQuad(RenderPass pass) {
        var indexInfo = RenderSystem.getSequentialBuffer(PrimitiveTopology.QUADS);
    ...
    private record Panel(float x, float y, float width, float height, float radius, boolean light) {
    }
}
